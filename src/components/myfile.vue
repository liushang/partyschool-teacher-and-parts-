    ...
    <li class="file_li" v-for="(i,index) in file"
      ...
      <div class="file_all">
        ...
        <div class="file-sin file-time">
          <div>{{i.time}}</div>
          <div class="short-tip" v-if="i.from==='办公室'">5/9</div>
          <div v-else class="short-tip short-state" :class="[{fstate0:(i.look===0)},{fstate1:(i.mestate===1)},{fstate2:(i.mestate===-1)},{fstate3:(i.look===1&&i.mestate===0)}]">{{i.look===0?'未读':(i.mestate===0?'已读':(i.mestate===1?'已同意':'已拒绝'))}}</div>
        </div>
      </div>
    </li>
  </div>
      ...
      stl(){
        switch (this.$route.path) {
          ...
          case '/myfile/filedone':
            this.file = this.files.files.filter((item) => item.mestate !== 0 && item.from !== '办公室')
            break
          case '/myfile/filenot':
            this.file = this.files.files.filter((item) => item.mestate === 0 && item.from !== '办公室')
            break
